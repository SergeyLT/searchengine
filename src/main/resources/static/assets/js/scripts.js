            ...
            action: function(result, $this){
                if (result.result){
                    ...
                    result.statistics.detailed.forEach(function(site){
                        ...
                        var time = new Date(site.statusTime * 1000 + timeZoneOffset * 60000);
                        $blockSiteExample.find('.Statistics-description')
                            .html('<div class="Statistics-option"><strong>Status time:</strong> ' +
                                ...
                                (site.error === undefined ? '' : '</div><div class="Statistics-option Statistics-option_error"><strong>Error:</strong> ' +
                                 site.error + '</div>') +
                                '')
    ...
    return {
        init: function(){
            ...
            $btnCheck.on('click', function(e){
                var $this = $(this);
                if ($this.data('send') === 'startIndexing' && !$this.next('.API-success').length) {
                    $this.after('<div class="API-success">Ожидание начала индексации</div>');
                }
                if (!$this.data('send')) {
                    shiftCheck($this);
                }
